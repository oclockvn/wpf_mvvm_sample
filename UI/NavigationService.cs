        ...
        public void NavigateTo(PageRoute page)
        {
            ...
            switch (page)
            {
                case PageRoute.Login:
                    ...
                    break;
                case PageRoute.Employee:
                    content = serviceProvider.GetRequiredService<EmployeePage>();
                    break;
                default:
                    throw new NotSupportedException($"Destination {page} is not supported");
            }

            var root = FindChild<Frame>(App.Current.MainWindow, "root");
            root.Navigate(content);
        }
